...
FINAL_PNG = OUT / "Figure2_spatiotemporal_traffic_environmental_dynamics.png"
PREVIEW_PNG = OUT / "Figure2_spatiotemporal_traffic_environmental_dynamics_preview.png"
AUDIT_CSV = OUT / "Figure2_plot_audit.csv"
...
def load_and_validate() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict]:
    ...
    panel = panel.loc[panel["split"].eq("test")].copy()
    ...
    scenario = pd.read_parquet(scenario_path, columns=scenario_columns)
    scenario = scenario.loc[scenario["split"].eq("test")].copy()
    ...
    if panel.duplicated(["node_id", "timestamp_local"]).any():
        raise AssertionError("Duplicate node-hour rows in the frozen test panel")
    if scenario.duplicated(["node_id", "timestamp_local"]).any():
        raise AssertionError("Duplicate node-hour rows in the frozen scenario labels")
    if panel["node_id"].nunique() != 51:
        raise AssertionError("The frozen test panel does not contain 51 nodes")
    node_hours = panel.groupby("timestamp_local")["node_id"].nunique()
    if not node_hours.eq(51).all():
        raise AssertionError("The test-period node × time grid is incomplete")
    ...
    if len(timestamps) > 1:
        ...
        if not deltas.eq(pd.Timedelta(hours=1)).all():
            raise AssertionError("The frozen test-period timestamps are not a continuous hourly index")
    ...
    if merged[["S2_rain", "S7_elevated_aod", "S8_rain_elevated_atmospheric_pollution"]].isna().any().any():
        raise AssertionError("Frozen scenario labels failed to join to all test node-hours")
    ...
    if len(merged) != expected_rows:
        raise AssertionError(f"Unexpected test grid rows: {len(merged)} != {expected_rows}")
    if merged["current_speed_input"].isna().any():
        raise AssertionError("Frozen current_speed_input contains test-period NA values")
    ...
    summary = {
        ...
        **feature_info,
        ...
        "test_start": str(hourly.index.min()),
        "test_end": str(hourly.index.max()),
        ...
        "source_speed_na": int(merged["current_speed_input"].isna().sum()),
...
def make_figure(merged: pd.DataFrame, hourly: pd.DataFrame, order: pd.DataFrame) -> plt.Figure:
    ...
    ax_speed.set_xlabel("Frozen test period (local time)", labelpad=7)
    ax_speed.xaxis.set_major_locator(mdates.DayLocator(interval=2))
    ax_speed.xaxis.set_major_formatter(mdates.DateFormatter("%d %b"))
...
def write_documentation(summary: dict, output_records: list[dict]) -> None:
    (OUT / "Figure2_audit_summary.json").write_text(
        json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    with (OUT / "Figure2_output_manifest.csv").open("w", newline="", encoding="utf-8-sig") as handle:
...
**Figure 2. Spatiotemporal traffic and environmental dynamics during the frozen test period.** The heatmap shows traffic speed across the verified 0.000–{summary['corridor_km_max']:.3f} km corridor from {summary['test_start']} to {summary['test_end']} (local time). Traffic values are the frozen `current_speed_input` used by E1–E5; cells identified by `missing_mask=1` are displayed separately in grey rather than showing their causal last-observation-carried-forward values. The aligned strips show the corridor-median frozen `precipitation` predictor and corridor-median frozen `aerosol_optical_depth` predictor. Faint shading identifies hours when at least 50% of the 51 nodes carry the corresponding frozen scenario label, with Compound taking precedence over Rain and Elevated AOD for display. Shading reports temporal coincidence only and does not imply that environmental conditions caused the observed traffic pattern.
"""
...
The plotted rainfall and AOD strips use corridor medians of the actual frozen predictor columns. No scenario threshold was recalculated from the test set.

## Frozen test grid

- Period: {summary['test_start']} to {summary['test_end']}
- Hours: {summary['test_hours']}
...
`Figure2_plot_audit.csv` contains every plotted node-hour, source value, plotted value, environmental value, and frozen scenario assignment.
"""
    (OUT / "README_Figure2.md").write_text(readme, encoding="utf-8")
